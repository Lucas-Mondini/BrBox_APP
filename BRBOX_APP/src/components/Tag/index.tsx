...
export default function Tag({tag, specificStyle, large, home, topTags, showName, noEvaluations, showTotalVotes, callback, extraStyles}: TagProps)
{
  // @ts-ignore
      ...
      <View style={[{height: large ? 25 : 20, flexDirection: "row"}, !noEvaluations ? {width: large ? 110 : home ? 40 : 100} : {paddingHorizontal: 5, backgroundColor: bg}]}>
        <View style={[{
          ...
          <Text style={[{
            ...
            {showTotalVotes && <Text style={[styles.tagText, large ? styles.tagTextLarge : styles.tagTextSmall]}>  {formatVotes(tag.count || tag.total)}</Text>}
            {showName && <Text style={[styles.tagText, large ? styles.tagTextLarge : styles.tagTextSmall, {paddingLeft: 5}]}>  {tag.name}</Text>}
            </Text>
        </View>

        {!noEvaluations && <>
          <View style={{backgroundColor: config.darkGreen, flex: tag.upVotes}}/>
